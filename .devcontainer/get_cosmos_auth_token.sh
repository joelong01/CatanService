...
if [ -z "$COSMOS_COSMOS_RESOURCEGROUP" ] || [ -z "$COSMOS_ACCOUNT_NAME" ]; then
...
keys=$(az cosmosdb keys list --name "$COSMOS_ACCOUNT_NAME" --resource-group "$COSMOS_COSMOS_RESOURCEGROUP" 2>/dev/null)
if [[ -z $keys ]]; then
